...
pub fn xref_and_trailer(
    input: ParserInput, reader: &Reader, max_xref_entries: Option<usize>,
) -> crate::Result<(Xref, Dictionary)> {
    let xref_trailer = map(
        pair(|input| xref(input, max_xref_entries), trailer),
        |(xref, trailer)| {
            let mut xref = xref?;
            xref.size = trailer
                .get(b"Size")
                ...
                .map(|(_, obj)| {
                    let res = match obj {
                        Object::Stream(stream) => {
                            decode_xref_stream_with_limits(stream, reader.max_decompressed_size, max_xref_entries)
                        }
